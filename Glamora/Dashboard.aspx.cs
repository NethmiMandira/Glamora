...
namespace Glamora
{
    public partial class Dashboard : Page
    {
        ...
        {
            // Check authentication. Redirect unauthenticated users to the Login page.
            // During local development allow a test session so the dashboard is reachable without full auth.
            if (Session["UserId"] == null)
            {
                // If running locally, populate a dev/test user id from config so developers can access pages quickly.
                if (Request.IsLocal)
                {
                    try
                    {
                        var devUser = System.Configuration.ConfigurationManager.AppSettings["AuthTestUserId"] ?? "dev";
                        Session["UserId"] = devUser;
                    }
                    catch
                    {
                        Session["UserId"] = "dev";
                ...
                {
                    // Production / remote requests should go to the login page (Splash is a marketing page).
